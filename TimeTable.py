...
class Data:
    ...
    Time_Avilable = [["MT1", "Mo, We, Fr 09:00 - 10:00"],
                     ["MT2", "Mo, We, Fr 10:00 - 11:00"],
                     ["MT3", "Tu, Th, Su 09:00 - 10:30"],
                     ["MT4", "Tu, Th, Su 10:30 - 12:00"]]
    ...
    def __init__(self):
        ...
        course1 = Courses("C1", "MATH1", [self._instructors[0], self._instructors[1]], 25)
        course2 = Courses("C2", "AI", [self._instructors[0], self._instructors[1], self._instructors[2]], 35)
        course3 = Courses("C3", "MATH2", [self._instructors[0], self._instructors[1]], 25)
        course4 = Courses("C4", "OS", [self._instructors[2], self._instructors[3]], 30)
        course5 = Courses("C5", "CO", [self._instructors[3]], 35)
        course6 = Courses("C6", "DB", [self._instructors[0], self._instructors[2]], 45)
        course7 = Courses("C7", "IR", [self._instructors[1], self._instructors[3]], 45)
...
class genatic_algorithm:
    ...
    def differentially_evolve(self, popu):
    ...
    def crossover(self, popu):
        ...
        while i < POPULATION_SIZE:
            schedule1 = self.select_tournament_population(popu)[0].get_schedules()[0]
            schedule2 = self.select_tournament_population(popu)[0].get_schedules()[1]
            schedule3 = self.select_tournament_population(popu)[1]
            schedule4 = self.select_tournament_population(popu)[0].get_schedules()[3]
            crossover_pop.get_schedules().append(self.crossover_schedule(schedule3, schedule4))
            i += 1
    ...
    def crossover_schedule(self, schedule3, schedule4):  #get new childe from restored  2 schduler parent
        crossoverSchedule = schedule().initialize()
        for i in range(0, len(crossoverSchedule.get_classes())):
            if (random() > 0.5):
                crossoverSchedule.get_classes()[i] = schedule4.get_classes()[i]
            else:
                crossoverSchedule.get_classes()[i] = schedule3.get_classes()[i]
            if schedule4.get_fitness() > crossoverSchedule.get_fitness(): # compares with the 4th parent
                crossoverSchedule.get_classes()[i] = schedule4.get_classes()[i]
    ...
    def select_tournament_population(self, popu): # to select parents
        ...
        schedule3 = tournament_pop.get_schedules()[2]
        schedule4 = tournament_pop.get_schedules()[3]
        difference = schedule1.get_fitness() - schedule2.get_fitness() # p1 - p2
        weightedDifference = schedule3.get_fitness() + (0.5 * difference) #p3 * F(p1 - p2)

        for i in range(POPULATION_SIZE): # get a child with the fitness == weightedDifference
            if popu.get_schedules()[i].get_fitness() == weightedDifference:
                third = popu.get_schedules()[i]



        # print (len(tournament_pop.get_schedules()))
        return [tournament_pop, third]
...
while (population.get_schedules()[0].get_fitness() != 1.0):
    genrationNumber += 1
    print("\n> Generation # " + str(genrationNumber))
    population = genatic.differentially_evolve(population)
    population.get_schedules().sort(key=get_sort_key, reverse=True)
    display.print_generation(population)
...
tree=ttk.Treeview(win, column=("c1", "c2", "c3", "c4", "c5", "c6"), show='headings', height=7)
tree.column("# 1",anchor=CENTER, stretch=NO, width=100)
tree.heading("# 1", text="Class #")

tree.column("# 2", anchor=CENTER, stretch=NO)
tree.heading("# 2", text="Dept")

tree.column("# 3", anchor=CENTER, stretch=NO)
tree.heading("# 3", text="Course (Number, Max# of Students)")

tree.column("# 4", anchor=CENTER, stretch=NO)
tree.heading("# 4", text="Room (Capacity)")

tree.column("# 5", anchor=CENTER, stretch=NO)
tree.heading("# 5", text="Instructor (ID)")

tree.column("# 6", anchor=CENTER, stretch=NO)
tree.heading("# 6", text="Meeting Time (ID)")

# Insert the data in Treeview widget
tree.insert('', 'end',text="0",values=('0','CS','ST-121 (C1, 120)', 'H2 (350)', 'DR/ MARWA ABD EL FATTAH (I2)', 'From 11:00 AM to 1:30 PM (LEC2)'))
tree.insert('', 'end',text="1",values=('1','CS','CS-314 (C3, 200)', 'H1 (400)', 'DR/ MARWA ABD EL FATTAH (I2)', 'From 4:00 PM to 6:00 PM (LEC4)'))
tree.insert('', 'end',text="2",values=('2','IS','CS-214 (C2, 150)', 'H4 (150)', 'DR/ LAILA ABD EL HAMID (I3)', 'From 2:00 PM to 4:00 PM (LEC3)'))
tree.insert('', 'end',text="3",values=('3','IS','IS-241 (C4, 100)', 'H4 (150)', 'DR/ MANAL ABD EL KADER (I4)', 'From 4:00 PM to 6:00 PM (LEC4)'))
tree.insert('', 'end',text="4",values=('4','IS','IT-222 (C5, 140)', 'H2 (350)', 'DR/ MANAL ABD EL KADER (I4)', 'From 2:00 PM to 4:00 PM (LEC3)'))
tree.insert('', 'end',text="5",values=('5','IT','IS-211 (C6, 198)', 'H3 (200)', 'DR/ AMR S. GHONIEM (I1)', 'From 2:00 PM to 4:00 PM (LEC3)'))
tree.insert('', 'end',text="6",values=('6','IT','IS-444 (C7, 120)', 'H4 (150)', 'DR/ MANAL ABD EL KADER (I4)', 'From 9:00 AM to 11:00 AM (LEC1)'))
